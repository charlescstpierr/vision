...
- **Perte de travail sur un run**: résolu côté serveur par trois garde-fous. Le message `cancel` interrompt le run en vol ; un délai (10 min par défaut, `runTimeoutMs`) l'interrompt tout seul pour qu'un agent bloqué ne garde pas l'`OpLock` indéfiniment ; et le diff non décidé est désormais une valeur à l'échelle du projet, qui survit à la fermeture du panneau et est rejouée à la reconnexion. Reste ouvert: il ne survit pas au redémarrage du serveur (l'historique d'annulation non plus, cf. `RunHistory`, en mémoire et plafonné à 50 entrées).
